<template>
    <div>
      ...
      <el-table :data="tableList" :default-sort="{ prop: 'updatedDt', order: 'descending'}">
        <el-table-column label="ID" prop="newsArticleId" sortable></el-table-column>
        <el-table-column label="新闻标题" prop="newsArticleTitle">
        </el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      search() {
        HomeNewsReportApi.queryPage(this.query).then(data => {
      ...
      toDelete(id) {
        this.$confirm('', '请确认删除?', {}).then(() => {
          HomeNewsReportApi.remove(id).then(data => {
            console.log(data)
